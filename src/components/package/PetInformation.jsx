...
export default function PetInformation({ pet }) {
  ...
  const [image, getImage] = useGetImage();
  const [traits, traitSize, traitStatus, getTraits] =
    useFetchItem("archetopia_traits");
  const [skills, skillSize, skillStatus, getskills] =
    useFetchItem("archetopia_skills");

  const loadData = () => {
    startTransition(async () => {
      const traits = await getTraits(pet.PETID);
      const skills = await getskills(pet.PETID);
      const image = await getImage(pet.Image);
      return [traits, image, skills];
    });
    return;
  };
        ...
        <section className="flex-1 rounded p-5 h-full overflow-y-auto">
          <main className="h-full flex flex-col">
            <section className="lg:w-full flex flex-col items-center mb-5">
              ...
              <figure className="w-fit aspect-square rounded overflow-hidden">
                <img
                  src={image ? image : pet.Name}
                  alt={"..."}
                  className="size-40 object-cover object-center rounded-t"
                ></img>
              </figure>
            </section>
